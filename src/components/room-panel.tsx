...
import { usePanelResize, type RoomLayout } from '@/lib/use-panel-resize';
import { api } from '@/lib/client-api';
import { connectLive, emitLive, emitWhenConnected, onLive, onLiveConnect, liveConnected, liveSocketId } from '@/lib/live';
import { toast } from '@/components/toast';
import { ArrowLeft, Globe, Lock, Users as UsersIcon, UserPlus, LogOut, LockOpen, Ban, Copy, User, Music2, Film, Hammer, Trash2, LayoutGrid } from 'lucide-react';
...
export function RoomPanel({ jamId, onBack }: { jamId: string; onBack: () => void }) {
  ...
  const { gridRef, gridStyle, layout, setLayout, beginResize, beginVertical } = usePanelResize(jam?.kind === 'MUSIC');
  ...
  return (
    <div className="room authkit-room" style={{ marginTop: 24 }}>
      ...
      <div className="room-actions-row">
        <div className="room-layouts">
          <span className="room-layouts-label"><LayoutGrid size={13} /> {t('room.layouts')}</span>
          {(['classic', 'stack', 'focus'] as RoomLayout[]).map((l) => (
            <button key={l} type="button" className={`pill-sm ${layout === l ? 'active' : ''}`} onClick={() => setLayout(l)} title={t(`room.layout_${l}`)}>
              {t(`room.layout_${l}`)}
            </button>
          ))}
        </div>
      ...
      <div className={`room-grid rl-${layout}`} ref={gridRef} style={gridStyle}>
        <aside className="room-col-members">
          ...
          {showInvite && (
            <div className="room-foot-actions" style={{ paddingTop: 12, flexWrap: 'wrap' }}>
              {friends.length === 0 ? (
        ...
        {jam.kind === 'MUSIC' && (
          <>
            ...
            <aside className="room-col-music">
              <MusicPlayer jamId={jam.id} canOwner={isOwner} miniHost={((jam.members.find((m) => m.id === me?.id)?.role ?? '') === 'MINI_HOST')} />
            </aside>
          </>
        )}
      </div>
