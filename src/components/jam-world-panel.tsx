...
export function JamWorldPanel({ jamId, jamName, kind, description, members, messages, isOwner }: JamWorldProps) {
  ...
  return (
    <section className={`jam-world-panel jam-world-${world.tone}`}>
      ...
      <div className="jam-world-actions"><button type="button" className="btn btn-violet pill-sm" onClick={() => setOpen(true)} title="Save the current room context and create a shareable link"><Camera size={14} /> Capture moment</button><small className="jam-world-action-hint">Saves the room snapshot, not an audio/video recording.</small>{isOwner && <span className="jam-world-owner"><Crown size={13} /> Host world</span>}</div>
