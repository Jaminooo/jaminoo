...
function serialize(): string {
  ...
  const p = new URLSearchParams();
    if (s.product === 'music' || s.product === 'video' || s.product === 'cinema' || s.product === 'tweet' || s.product === 'anime') p.set('hub', s.product);
...
function hydrateFromURL() {
  ...
  const validHub: HubProduct = hub === 'community' || hub === 'music' || hub === 'video' || hub === 'cinema' || hub === 'anime' || hub === 'tweet' ? hub : hasCommunityRoute ? 'community' : 'home';
  s.setProduct(validHub);
  if (validHub === 'music' || validHub === 'video' || validHub === 'cinema' || validHub === 'anime' || validHub === 'tweet') return;
  if (user && /^\d+$/.test(user)) {
