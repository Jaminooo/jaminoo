...
async function hasApprovedCreator(userId: number) {
...
export const POST = handle(async (req: Request) => {
  ...
  if (!mime) return err('Only valid PNG, JPG, WEBP, MP4, MOV or WEBM files are allowed');
  ...
  const extension = mime === 'image/png' ? 'png' : mime === 'image/webp' ? 'webp' : mime === 'image/jpeg' ? 'jpg' : mime === 'video/webm' ? 'webm' : mime === 'video/quicktime' ? 'mov' : 'mp4';
  ...
  await persistMediaBytes(record.id, buffer, mime);
