...
export const POST = handle(async (req) => {
  ...
  const draft = await prisma.draft.upsert({
    where: { userId: me.id },
    update: { text, mediaIds, ...(body.quotedTweetId === null ? { quotedTweetId: null } : quotedTweetId ? { quotedTweetId } : {}) },
    create: { userId: me.id, text, mediaIds, quotedTweetId: quotedTweetId ?? null },
  });
  return json({ draft: { id: draft.id, text: draft.text, mediaIds: safeJson(draft.mediaIds), quotedTweetId: draft.quotedTweetId, updatedAt: draft.updatedAt.toISOString() } });
});
