...
import { jamCinemaState, setJamCinemaState } from '@/lib/jam-cinema';
...
export const GET = handle(async (_req, { params }: Ctx) => {
  ...
  if (jam.kind !== 'MOVIE') return err('This jam has no cinema player', 400);
  if (!jam.members.some((member) => member.userId === me.id)) return err('You are not in this jam', 403);
  const data = await jamCinemaState(params.id, me.id, me.isAdmin);
...
export const PATCH = handle(async (req, { params }: Ctx) => {
  ...
  try {
    const data = await setJamCinemaState({
      jamId: params.id,
      viewerId: me.id,
      viewerIsAdmin: me.isAdmin,
      action,
      videoId: Number.isInteger(videoId) ? videoId : undefined,
      position: Number.isFinite(position) ? position : undefined,
    });
    if (!data) return err('Jam not found', 404);
    return json(data);
  } catch (error) {
    return err(error instanceof Error ? error.message : 'Could not update cinema playback', 400);
  }
});
